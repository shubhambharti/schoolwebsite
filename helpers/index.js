...
hbs.registerHelper("siteEmail", () => {
  return setting.email && setting.email !== ""
    ? setting.email
    : process.env.SITE_EMAIL;
});

hbs.registerHelper("sitePhone", () => {
  return setting.phone && setting.phone !== ""
    ? setting.phone
    : process.env.SITE_PHONE;
});

hbs.registerHelper("siteWhatsApp", () => {
  return setting.whatsapp && setting.whatsapp !== ""
    ? setting.whatsapp
    : process.env.SITE_WHATSAPP;
});

hbs.registerHelper("siteAddress", () => {
  return setting.address && setting.address !== ""
    ? setting.address
    : process.env.SITE_ADDRESS;
});

hbs.registerHelper("siteMap1", () => {
  return setting.map1 && setting.map1 !== ""
    ? setting.map1
    : process.env.SITE_GOOGLE_MAP1;
});

hbs.registerHelper("siteMap2", () => {
  return setting.map2 && setting.map2 !== ""
    ? setting.map2
    : process.env.SITE_GOOGLE_MAP2;
});

hbs.registerHelper("siteFacebook", () => {
  return setting.facebook && setting.facebook !== ""
    ? setting.facebook
    : process.env.SITE_FACEBOOK;
});

hbs.registerHelper("siteTwitter", () => {
  return setting.twitter && setting.twitter !== ""
    ? setting.twitter
    : process.env.SITE_TWITTER;
});

hbs.registerHelper("siteYoutube", () => {
  return setting.youtube && setting.youtube !== ""
    ? setting.youtube
    : process.env.SITE_YOUTUBE;
});

hbs.registerHelper("siteInstagram", () => {
  return setting.instagram && setting.instagram !== ""
    ? setting.instagram
    : process.env.SITE_INSTAGRAM;
});

hbs.registerHelper("siteLinkedin", () => {
  return setting.linkedin && setting.linkedin !== ""
    ? setting.linkedin
    : process.env.SITE_LINKEDIN;
});
...
hbs.registerHelper("isActiveFormHelper", (selected, value) => {
  ...
  else return String(selected) == String(value) ? "selected" : "";
});
...
hbs.registerHelper("isSuperAdminHelper", (session) => {
  return session && session.login && session.role === "Super Admin"
    ? true
    : false;
});
