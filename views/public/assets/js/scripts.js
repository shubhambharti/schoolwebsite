...
(function ($) {
  ...
  jQuery(document).on("ready", function () {
    ...
    // });
    $(window).on("load", function () {
      // This is the main preloader container element
      var pagePreloader = document.getElementById("page-preloader");

      // This is your text element
      var textWrapper = document.querySelector(".ml3");

      // This is your fadeOut function (slightly improved to start at full opacity)
      function fadeOut(el) {
        el.style.opacity = 1; // Start fade from full opacity
        var last = +new Date();
        var tick = function () {
          el.style.opacity = +el.style.opacity - (new Date() - last) / 500; // Fade out faster (500ms)
          last = +new Date();
          if (+el.style.opacity > 0) {
            (window.requestAnimationFrame && requestAnimationFrame(tick)) ||
              setTimeout(tick, 16);
      ...
      // Prepare the text by wrapping each letter in a <span> tag
      textWrapper.innerHTML = textWrapper.textContent.replace(
        /\S/g,
        "<span class='letter'>$&</span>"
      );
      ...
      anime
        .timeline({
          // loop: true has been removed so it only plays once
          // The `complete` callback ensures the fade-out happens AFTER the animation
          complete: function (anim) {
            // Target the main preloader container to fade everything out
            fadeOut(pagePreloader);
          },
        })
        .add({
          targets: ".ml3 .letter",
          opacity: [0, 1],
          easing: "easeInOutQuad",
          // DURATION & DELAY REDUCED for a faster animation (~1.5s total)
          duration: 800,
          delay: (el, i) => 50 * (i + 1),
        });
    });
    ...
    $(".service-slides-wrapper.owl-theme .owl-dots .owl-dot").click(
      function () {
